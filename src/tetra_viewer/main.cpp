...
int tetra_viewer::run_application(int argc, char** argv,ApplicationMode mode)
{
    ...
    if(world_mode){
        ...
        constexpr std::string_view terrain_field_threshold_prefix=
        ...
        for(int argument=1;argument<argc;++argument){
            ...
            }else if(value.starts_with(terrain_field_threshold_prefix)){
                double parsed{};
                if(!parse_argument_double(value.substr(
                       terrain_field_threshold_prefix.size()),parsed)||
                   parsed<0.0001||parsed>1.0e12){
                    fprintf(stderr,"terrain field pixel threshold must be in [0.0001,1e12]\\n");
                    return 2;
                }
                world_field_threshold_override=parsed;
            }else if(value.starts_with(terrain_limb_threshold_prefix)){
                double parsed{};
                if(!parse_argument_double(value.substr(
                       terrain_limb_threshold_prefix.size()),parsed)||
                   parsed<0.0001||parsed>1.0e12){
                    fprintf(stderr,"terrain limb pixel threshold must be in [0.0001,1e12]\\n");
                    return 2;
                }
                world_limb_threshold_override=parsed;
    ...
    if(world_mode){
        ...
        if(world_field_threshold_override)
            profile.field_error_pixel_threshold=*world_field_threshold_override;
        if(world_limb_threshold_override)
            profile.limb_error_pixel_threshold=*world_limb_threshold_override;
        ...
        world_runtime_startup=
            tetra_viewer::make_production_terrain_runtime_async(
                profile,world_lod_camera);
            ...
            if(!world_runtime&&world_runtime_startup.valid()&&
               world_runtime_startup.wait_for(std::chrono::seconds(0))==
                ...
                try{
                    ...
                    std::cerr<<"world startup failed: "<<error.what()<<'\n';
               ...
               !world_runtime->diagnostics().busy){
                ...
                if(timing.valid){
                    ...
                    std::cout
                        ...
                        <<",\"position_normal_hash_matches_cpu\":"
                        <<(gpu_extract.position_normal_hash_matches_cpu?"true":"false")
                        <<",\"gpu_position_normal_hash\":"
                ...
                if(!tetra_viewer::make_rgb8_image(
                   ...
                   !tetra_viewer::write_pgm(
                    ...
                    std::cout<<",\"terrain_lod\":{\"capture_kind\":\""
                             ...
                             <<runtime_status.retained_cache_bytes
                             <<",\"retained_conforming_bytes\":"
                             <<runtime_status.retained_conforming_bytes
                    ...
                    // Acceptance statistics describe the current settled
