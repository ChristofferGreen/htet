...
run_case() {
  local transport="$1" name="$2" camera="$3" yaw="$4" pitch="$5"
  local sun_azimuth="$6" sun_elevation="$7"
  local image="${output_dir}/${transport}-${name}.ppm"
  local evidence="${output_dir}/${transport}-${name}.jsonl"
  local probe=()
...
for transport in "${transports[@]}"; do
  ...
  run_case "${transport}" ground "0.5,0.72,0.78" 180 -14.3239 -103.1324 5
  run_case "${transport}" noon "0.5,0.72,0.78" 180 -14.3239 -103.1324 60
  run_case "${transport}" sunset "0.5,0.72,0.78" 180 -8 -103.1324 1
  run_case "${transport}" mountain-shadow "0.5,0.72,0.78" 180 -6 -75 5
  run_case "${transport}" flight "0.5,100.5,0.78" 180 -5 -103.1324 25
  run_case "${transport}" limb "0.5,5000.5,0.5" 180 -38.5 -103.1324 10
  run_case "${transport}" orbit "0.5,25000.5,0.5" 180 -88 -103.1324 35
  run_case "${transport}" terminator "0.5,25000.5,0.5" 180 -88 90 0
done
