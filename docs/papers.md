...
This is the single research index for Dygd. It combines the tetrahedral-subdivision bibliography with work on topology edits, persistent correspondence, hierarchy and traversal, terrain authoring and erosion, contact, adaptive dynamics, bonding, lighting, alternate material regimes, and representation management.
...
| H38 | 2013 | M. Scholz, J. Bender, C. Dachsbacher | **Level of Detail for Real-Time Volumetric Terrain Rendering** | A | 95 | 100 | 95 | 100 | Volumetric-terrain citation audit | Maintains a reversible longest-edge-bisection diamond front with split/coarsen hysteresis, caches immutable per-cell surfaces, and decouples topology from rendering through a second fixed-capacity GPU-buffer front assembled on a background thread. Each active tetrahedron is converted to four matching hexahedral lattices before modified Marching Cubes extraction, yielding crack-free cross-LOD surfaces without stitching and better triangle quality than direct simplex contouring. This is the immediate algorithmic predecessor to H34 and a close match for the viewer's camera-driven update path. | [PDF](../papers/hierarchy/2013-Level%20of%20Detail%20for%20Real-Time%20Volumetric%20Terrain%20Rendering.pdf) | [source](https://doi.org/10.2312/PE.VMV.VMV13.211-218) |
...
### Forward citation audit for H38

The 11 Google Scholar citations to the 2013 volumetric-terrain paper do not reveal another hierarchy algorithm that should displace H38 or H34. The useful adjacent works are:

| Work | Decision | Relevance |
|---|---|---|
| E. Galin et al. — **A Review of Digital Terrain Modeling** (2019) | Supporting survey | Broad taxonomy of terrain representations and generation methods; useful for terminology and coverage checks, but not an implementation prescription for tetrahedral LOD. |
| M. Becher et al. — **Feature-Based Volumetric Terrain Generation** (2017) | Core terrain-authoring precursor | Builds volumetric terrain from sparse curve features using diffusion and noise. It is directly relevant to editable caves, overhangs, arches, and other non-heightfield terrain even though it does not prescribe the tetrahedral hierarchy. |
| V. Shakaev — **Polygonizing Volumetric Terrains with Sharp Features** (2016) | Surface-extraction watchlist | Directly concerns preserving sharp terrain features during polygonization and may provide another surface-quality comparison. It does not appear to introduce a new reversible tetrahedral hierarchy. |
| V. Shakaev, N. Sadovnikova, D. Parygin — **View-Dependent Level of Detail for Real-Time Rendering of Large Isosurfaces** (2017) | Secondary LOD reference | On-topic, but the available abstract does not establish a distinct stronger mechanism than the diamond-front and cached-cell pipeline already covered by H38 and H34. Reassess if its full construction differs materially. |
| R. van Rüschen et al. — **Volumetric Terrain Rendering with WebGL** (2018) | Portability reference | Demonstrates browser-oriented volumetric terrain rendering; potentially useful for implementation constraints, but lower priority than the native real-time hierarchy papers. |
| R. Li — **Real-World Large-Scale Terrain Model Reconstruction and Real-Time Rendering** (2023) | Adjacent only | Focuses on reconstruction and high-fidelity rendering of real-world terrain rather than the editable volumetric tetrahedral hierarchy used here. |

The longest-edge n-section survey is already canonical as E17. The river-landscape thesis is procedural-landscape background, while the flight-simulator citation and repository-only citation are not actionable research additions.

### Newer terrain-authoring and erosion citation branch

The imported reading set is stored under `papers/terrain/`. Titles below link to the local PDF when a verified public copy was available; the adjacent DOI link remains the canonical publication record. Of the 33 selected works, 27 have been imported. Six recent or closed-access works have no verified public PDF as of 2026-08-25 and remain linked to their publication record: Timokhin and Mikhaylyuk (2020), Ma et al. (2025), Zhou et al. (2025), Zhang et al. (2026), Nilles et al. (2026), and Zhou et al. (2026). Similar titles, previews, and HTML landing pages were not substituted for the papers.

Terrain authoring and erosion are first-class project concerns: the tetrahedral hierarchy is the editable material representation on which these operations should eventually act. Forward citations from the terrain review and feature-based volumetric work identify the following high-value reading queue:

| Priority | Year | Work | Why it matters |
|:---:|---:|---|---|
| A | 2019 | A. Paris et al. — **[Terrain Amplification with Implicit 3D Features](../papers/terrain/2019-Terrain%20Amplification%20with%20Implicit%20Three-Dimensional%20Features.pdf)** ([DOI](https://doi.org/10.1145/3342765)) | Encodes geology and volumetric landforms as editable construction trees of implicit primitives, then combines Poisson placement, shape grammars, erosion, invasion percolation, and polygonization. This is a direct architectural reference for authored arches, cliffs, hoodoos, and caves over a large base terrain. |
| A | 2021 | A. Paris et al. — **[Synthesizing Geologically Coherent Cave Networks](../papers/terrain/2021-Synthesizing%20Geologically%20Coherent%20Cave%20Networks.pdf)** ([DOI](https://doi.org/10.1111/cgf.14420)) | Generates controllable cave skeletons from geological fields and converts them into signed-distance construction trees with blending and warping. It supplies both a difficult volumetric test family and a plausible authoring model for subsurface structure. |
| A | 2023 | H. Schott et al. — **[Large-Scale Terrain Authoring through Interactive Erosion Simulation](../papers/terrain/2023-Large-Scale%20Terrain%20Authoring%20Through%20Interactive%20Erosion%20Simulation.pdf)** ([DOI](https://doi.org/10.1145/3592787)) | Makes stream-power erosion interactive by authoring uplift rather than final elevation, with incremental constraints, warping, copying, inverse reconstruction, and hydrologically consistent patch blending. Its incremental workflow is particularly relevant to live editing. |
| A | 2024 | M. Hartley et al. — **[Flexible Terrain Erosion](../papers/terrain/2024-Flexible%20Terrain%20Erosion.pdf)** ([DOI](https://doi.org/10.1007/s00371-024-03444-w)) | Applies particle-based erosion across height fields, voxel grids, material layers, and implicit surfaces. This is the most direct bridge from established erosion work to a non-heightfield tetrahedral or signed-distance representation. |
| A | 2024 | A. Jain et al. — **[Efficient Debris-Flow Simulation for Steep Terrain Erosion](../papers/terrain/2024-Efficient%20Debris-Flow%20Simulation%20for%20Steep%20Terrain%20Erosion.pdf)** ([DOI](https://doi.org/10.1145/3658213)) | Adds debris-flow erosion and deposition where ordinary fluvial erosion produces implausibly uniform steep slopes. Its unified GPU implementation and depression routing are relevant to both realism and interactive updates. |
| A | 2024 | A. Jain et al. — **[FastFlow: GPU Acceleration of Flow and Depression Routing for Landscape Simulation](../papers/terrain/2024-Graphics-Processor-Accelerated%20Flow%20and%20Depression%20Routing%20for%20Landscape%20Simulation.pdf)** ([DOI](https://doi.org/10.1111/cgf.15243)) | Reports a 5x flow-routing and 34x–52x depression-routing speedup on 1024x1024 terrain. The parallel routing primitives are strong candidates for erosion over an extracted surface or a surface-aligned subset of volume cells. |
| A | 2025 | Z. Yang et al. — **[Arenite: A Physics-Based Sandstone Simulator](../papers/terrain/2025-Physics-Based%20Sandstone%20Simulation.pdf)** ([DOI](https://doi.org/10.1145/3731201)) | Combines stress, wind and fluvial erosion, deposition, erodability painting, and vegetation to create fully three-dimensional arches, alcoves, hoodoos, and buttes. This is unusually close to the project's intended editable volumetric material direction. |
| A | 2025 | O. Argudo et al. — **[Terrain Descriptors for Landscape Synthesis, Analysis and Simulation](../papers/terrain/2025-Terrain%20Descriptors%20for%20Landscape%20Synthesis,%20Analysis%20and%20Simulation.pdf)** ([DOI](https://doi.org/10.1111/cgf.70080)) | Collects and compares geomorphological terrain metrics and identifies cheaper correlated proxies. These metrics can complement triangle-quality and normal-continuity measurements when judging whether generated terrain actually looks structurally plausible. |
| A | 2022 | S. D. Rajasekaran et al. — **[Perceived Terrain Realism Metric](../papers/terrain/2022-Perceived%20Terrain%20Realism%20Metric.pdf)** ([DOI](https://doi.org/10.1145/3514244)) | Links localized valleys, ridges, hollows, and related geomorphons to human realism judgments. Although formulated for elevation maps, it gives the evaluation harness a perceptual target beyond mesh smoothness alone. |
| B | 2024 | P. Tzathas et al. — **[Physically-Based Analytical Erosion for Fast Terrain Generation](../papers/terrain/2024-Physically%20Based%20Analytical%20Erosion%20for%20Fast%20Terrain%20Generation.pdf)** ([DOI](https://doi.org/10.1111/cgf.15033)) | Replaces long stream-power simulations with an analytical, multigrid-accelerated formulation and exposes terrain age as an authoring parameter. Initially heightfield-based, but valuable for fast previews and coarse control. |
| B | 2024 | Z. Yang et al. — **[Unerosion: Simulating Terrain Evolution Back in Time](../papers/terrain/2024-Unerosion%20-%20Simulating%20Terrain%20Evolution%20Back%20in%20Time.pdf)** ([DOI](https://doi.org/10.1111/cgf.15182)) | Reformulates fluvial, sedimentation, and thermal erosion backward in time. Its inverse operations offer a useful model for nondestructive authoring histories and editable alternatives rather than one-way terrain destruction. |
| B | 2026 | B. Huftier et al. — **[Terrain Synthesis and Authoring Based on Iso-Contours](../papers/terrain/2026-Terrain%20Synthesis%20and%20Authoring%20Based%20on%20Iso-Contours.pdf)** ([DOI](https://doi.org/10.1111/cgf.70389)) | Uses nested vector iso-contours, growth simulation, and marching-triangle reconstruction to provide compact direct editing while remaining convertible to ordinary elevation maps for erosion and amplification. |
| B | 2020 | P. Y. Timokhin, M. V. Mikhaylyuk — **[Method to Extract Isosurfaces on the GPU by Means of Programmable Tessellation](https://doi.org/10.1134/S036176882003010X)** *(public PDF unavailable)* | The only distinct extraction paper found through the sharp-feature citation branch. It is worth comparing with compute-based extraction, though its citation descendants do not reveal a stronger recent continuation. |

Following the most relevant works forward one more generation reveals six additional papers worth tracking:

| Year | Work | Citation path and relevance |
|---:|---|---|
| 2025 | B. Venu, A. Bosák, J. R. Padrón-Griffe — **[Procedural Multiscale Geometry Modeling Using Implicit Surfaces](../papers/terrain/2025-Procedural%20Multiscale%20Geometry%20Modeling%20Using%20Implicit%20Surfaces.pdf)** ([DOI](https://doi.org/10.1111/cgf.70233)) | Cites *Terrain Amplification*. Synthesizes particulate, fibrous, porous, and laminar volumetric structures on demand with implicit surfaces and sphere tracing. It broadens terrain authoring into material-scale structure, appearance, and eventual mechanical heterogeneity. |
| 2024 | X. Nie et al. — **[Realistic Simulation of Dissolution Process on Rock Surface](../papers/terrain/2024-Realistic%20Simulation%20of%20Dissolution%20on%20Rock%20Surfaces.pdf)** ([DOI](https://doi.org/10.3390/a17100466)) | Cites *Terrain Amplification* and the cave work. Simulates dissolution, runoff, deposition, evaporation, and lateral erosion at interactive rates to form pits, ditches, and stone forests. This is directly relevant to editable karst material. |
| 2024 | A. Gouy et al. — **[KarstNSim: A Graph-Based Method for 3D Geologically-Driven Simulation of Karst Networks](../papers/terrain/2024-Geologically%20Driven%20Three-Dimensional%20Karst%20Network%20Simulation.pdf)** ([DOI](https://doi.org/10.1016/j.jhydrol.2024.130878)) | Cites the cave-network paper. Generates varied three-dimensional conduit graphs using geology-aware anisotropic costs, hydrology, fractures, permeability, and solubility, with geometric and topological validation against real networks. |
| 2025 | F. Miville et al. — **[pyKasso: An Open-Source Three-Dimensional Discrete Karst Network Generator](../papers/terrain/2025-Open-Source%20Three-Dimensional%20Karst%20Network%20Generator.pdf)** ([DOI](https://doi.org/10.1016/j.envsoft.2025.106362)) | Cites the cave-network paper. Provides an open implementation based on anisotropic fast marching over geological models and uncertain inlet, outlet, fracture, and base-level data. It may be the most immediately reusable cave-layout generator. |
| 2025 | G. M. Garcia, A. Richard, M. Olivares-Mendez — **[PLUME: Procedural Layer Underground Modeling Engine](../papers/terrain/2025-Procedural%20Layer%20Underground%20Modeling%20Engine.pdf)** ([DOI](https://doi.org/10.1109/ISPARO66239.2025.11436893)) | Cites the cave-network paper. An open-source generator for varied underground environments, oriented toward simulation and robotics; useful as a test-scene and authoring reference even if its geology is less rigorous. |
| 2026 | A. M. Nilles, L. Günther, S. Müller — **[Real-Time Desertscapes Simulation with Reptation and Divergence-Free Wind Fields](https://doi.org/10.1007/978-3-032-07623-6_6)** *(public PDF unavailable)* | Cites *Flexible Terrain Erosion*. Extends the representation-independent erosion branch into real-time aeolian terrain evolution and should be checked for update-locality and GPU scheduling ideas. |

#### Third-generation forward citations

Querying all 19 works above together produced 63 distinct citing records. Nineteen are papers already represented in the chain, leaving 44 external descendants. The following 14 add a distinct technique or evaluation direction:

| Priority | Year | Work | New contribution to track |
|:---:|---:|---|---|
| A | 2026 | N. McDonald, G. Cordonnier — **[Stochastic Geomorphological Transport for Terrain Erosion Simulation](../papers/terrain/2026-Stochastic%20Geomorphological%20Transport%20for%20Terrain%20Erosion%20Simulation.pdf)** ([DOI](https://doi.org/10.1145/3811336)) | Replaces restrictive stream-power transport with a parallel stochastic particle formulation grounded in momentum conservation. It produces basins, braided rivers, meanders, deltas, floods, and rockfall-like transport without grid-direction artifacts. The current regular-grid implementation takes about 51 ms per 1024x1024 iteration and one to ten minutes per complete authoring run, so its value is the transport model and parallel structure rather than immediate frame-by-frame editing. |
| A | 2024 | H. Schott et al. — **[Terrain Amplification Using Multi-Scale Erosion](../papers/terrain/2024-Terrain%20Amplification%20Using%20Multi-Scale%20Erosion.pdf)** ([DOI](https://doi.org/10.1145/3658200)) | Amplifies coarse authored terrain using thermal erosion, stream-power erosion, and deposition at several spatial scales. It directly continues the interactive-uplift work and offers a way to add hydrologically consistent detail without asking the artist to author every scale. |
| A | 2024 | A. M. Nilles et al. — **[3D Real-Time Hydraulic Erosion Simulation Using Multi-Layered Heightmaps](../papers/terrain/2024-Three-Dimensional%20Real-Time%20Hydraulic%20Erosion%20Using%20Multi-Layered%20Heightmaps.pdf)** ([DOI](https://doi.org/10.2312/VMV.20241211)) | Extends real-time hydraulic erosion beyond a single-valued height field by using multiple layers. It is an important comparison representation for overhangs and caves: less general than tetrahedral material, but potentially far cheaper for terrain-dominant regions. |
| A | 2026 | C. Zhou, X. Nie, G. Yang — **[Real-Time Simulation of Hydraulic Erosion Based on Soil and Water Conservation Theory and Physical Calculation](https://doi.org/10.4208/csiam-am.so-2025-0068)** *(public PDF unavailable)* | Adds splash, incision, lateral erosion, sedimentation, and environmental factors while using volume maps to accelerate particle-based fluid-solid interaction. This is a direct reference for coupling real-time erosion particles to volumetric material. |
| A | 2024 | A. Jain, A. Sharma, K. S. Rajan — **[Learning-Based Infinite Terrain Generation with Level of Detailing](../papers/terrain/2024-Learning-Based%20Infinite%20Terrain%20Generation%20with%20Level%20of%20Detail.pdf)** ([DOI](https://doi.org/10.1109/3DV62453.2024.00077)) | Combines image completion, progressive super-resolution, and quadtree-oriented training to generate unbounded terrain whose detail tiles match at hierarchy boundaries. The topology is heightfield/quadtree based, but its learned refinement and seam-loss design are relevant to camera-driven detail synthesis. |
| A | 2023 | A. Paris et al. — **[Authoring and Simulating Meandering Rivers](../papers/terrain/2023-Authoring%20and%20Simulating%20Meandering%20Rivers.pdf)** ([DOI](https://doi.org/10.1145/3618350)) | Evolves controllable vector river networks with curvature-based migration, cutoffs, avulsions, oxbow lakes, and terrain constraints at interactive rates. It provides an explicit persistent feature graph that could drive erosion and refinement in the volumetric hierarchy. |
| A | 2023 | G. Cordonnier et al. — **[Forming Terrains by Glacial Erosion](../papers/terrain/2023-Forming%20Terrains%20by%20Glacial%20Erosion.pdf)** ([DOI](https://doi.org/10.1145/3592422)) | Couples learned high-order ice flow, multiscale advection, glacial/interglacial cycles, and debris transport to generate U-shaped valleys, fjords, and glacial lakes. It expands the process vocabulary beyond water-only erosion and supplies characteristic large-scale validation shapes. |
| A | 2023 | J. Löchner et al. — **[Interactive Authoring of Terrain Using Diffusion Models](../papers/terrain/2023-Interactive%20Terrain%20Authoring%20Using%20Diffusion%20Models.pdf)** ([DOI](https://doi.org/10.1111/cgf.14941)) | Offers style selection, feature sketches, editable imported terrain, and resolution amplification with source code and pretrained models. Its heightfield output is not the final representation, but the controls and learned prior can generate targets or coarse fields for volumetric amplification. |
| A | 2025 | Y. Liu, B. Beneš — **[Single-Shot Example Terrain Sketching by Graph Neural Networks](../papers/terrain/2025-Single-Shot%20Example%20Terrain%20Sketching%20Using%20Graph%20Neural%20Networks.pdf)** ([DOI](https://doi.org/10.1111/cgf.15281)) | Learns from one example and preserves the topology of user-sketched ridge and valley graphs. This is a useful model for turning sparse persistent feature networks into plausible terrain while retaining artist intent. |
| B | 2025 | K. Higo et al. — **[TerraFusion: Joint Generation of Terrain Geometry and Texture Using Latent Diffusion Models](../papers/terrain/2025-Joint%20Terrain%20Geometry%20and%20Texture%20Generation%20Using%20Latent%20Diffusion.pdf)** ([DOI](https://doi.org/10.1016/j.vrih.2025.11.001)) | Jointly generates correlated height and surface appearance and accepts sketch control. It is more relevant to authored material appearance than tetrahedral topology, but avoids the geometry/texture mismatch of separate generation pipelines. |
| B | 2026 | C. Zhang et al. — **[Infinite Details: A Cartography-Informed Parametric Method for Constructing Realistic 3D Natural Scenes](https://doi.org/10.1016/j.jag.2026.105416)** *(public PDF unavailable)* | Organizes global entities and context-constrained local detail through a semantic tree, spatial layout, and model placement, with perceptual validation. This is a possible semantic-detail layer above the material hierarchy. |
| B | 2025 | Y. Zhou et al. — **[ForwardTerrain: One-Pass Terrain Modeling Through Mid-Air Sketching Without Backtracking](https://doi.org/10.1016/j.cag.2025.104468)** *(public PDF unavailable)* | Explores a direct gestural authoring workflow. The interaction design may matter for future sculpting tools even if its terrain representation is not itself reusable. |
| B | 2026 | L. Ma et al. — **[From Data Modification to Preservation: Evolution of DEM Depression Processing Algorithms in Hydrological Modelling](../papers/terrain/2026-Digital%20Elevation%20Model%20Depression%20Processing%20for%20Hydrological%20Modelling.pdf)** ([DOI](https://doi.org/10.1007/s12665-026-12971-9)) | Reviews depression routing, data preservation, practical performance beyond asymptotic complexity, binary-tree encodings, and parallelization. It is useful for choosing the water-routing substrate behind erosion rather than inventing another local-minimum workaround. |
...
|:---:|---:|---|---|
| A | 2020 | S. Hillaire — **[A Scalable and Production Ready Sky and Atmosphere Rendering Technique](../papers/atmosphere/2020-Scalable%20Production%20Ready%20Sky%20and%20Atmosphere%20Rendering.pdf)** ([DOI](https://doi.org/10.1111/cgf.14050), [MIT reference implementation](https://github.com/sebh/UnrealEngineSkyAtmosphere)) | Selected realtime architecture: compact transmittance, multiple-scattering, sky-view, and aerial-perspective tables with dynamic parameters and continuous ground-to-space viewing. |
| A | 2008 | E. Bruneton, F. Neyret — **[Precomputed Atmospheric Scattering](../papers/atmosphere/2008-Precomputed%20Atmospheric%20Scattering.pdf)** ([DOI](https://doi.org/10.1111/j.1467-8659.2008.01245.x), [tested BSD implementation](https://ebruneton.github.io/precomputed_atmospheric_scattering/)) | Higher-cost correctness oracle for multiple scattering, aerial perspective, and terrain shadows visible as atmospheric light shafts. |
| A | 2021 | A. Wilkie et al. — **[A Fitted Radiance and Attenuation Model for Realistic Atmospheres](../papers/atmosphere/2021-Fitted%20Radiance%20and%20Attenuation%20Model%20for%20Realistic%20Atmospheres.pdf)** ([DOI](https://doi.org/10.1145/3450626.3459758)) | A measured-data fitted alternative with finite-distance in-scattering and attenuation. It is a useful terrestrial appearance and visibility oracle, but not a dynamic arbitrary-planet replacement for Hillaire. |
| A | 2022 | C. Breyer, T. Zirr — **[Planetary Shadow-Aware Distance Sampling](../papers/atmosphere/2022-Planetary%20Shadow-Aware%20Distance%20Sampling.pdf)** ([DOI](https://doi.org/10.2312/sr.20221152)) | Analytically intersects a cylinder fitted to the planet's shadow, discards zero-contribution shadowed intervals, and samples the remaining intervals uniformly in optical-depth space. This directly targets dawn/dusk undersampling; irregular terrain shadows still require the per-sample shadow-map test. |
| A | 2023 | Z. Velinov, K. Mitchell — **[Collimated Whole Volume Light Scattering in Homogeneous Finite Media](../papers/atmosphere/2023-Collimated%20Whole%20Volume%20Light%20Scattering%20in%20Homogeneous%20Finite%20Media.pdf)** ([DOI](https://doi.org/10.1109/TVCG.2021.3135764)) | Integrates and importance-samples whole-path single-scattered collimated light under occlusion. Its finite homogeneous media differ from a spherical atmosphere, but its variance and shaft treatment are directly relevant to mountain-shadowed haze. |
| A | 2024 | S. Schneegans et al. — **[Physically Based Real-Time Rendering of Atmospheres using Mie Theory](../papers/atmosphere/2024-Physically%20Based%20Real-Time%20Rendering%20of%20Atmospheres%20Using%20Mie%20Theory.pdf)** ([DOI](https://doi.org/10.1111/cgf.15010)) | Establishes the limitation of generic Henyey-Greenstein aerosols for alien skies and motivates a later tabulated spectral Mie mode, especially for the Martian blue sunset. |
| A oracle | 2026 | V. Leroy et al. — **[Eradiate: An Accurate and Flexible Radiative Transfer Model for Earth Observation and Atmospheric Science](../papers/atmosphere/2026-Eradiate%20Accurate%20and%20Flexible%20Radiative%20Transfer.pdf)** ([DOI](https://doi.org/10.5194/gmd-19-4289-2026), [code](https://github.com/eradiate/eradiate)) | Open-source three-dimensional Monte Carlo transport built on Mitsuba 3, with spectral and polarization modes and scientific validation. It is far too expensive for runtime use but is a stronger external reference-image oracle than another realtime approximation. |
| B | 2016 | E. Bruneton — **[A Qualitative and Quantitative Evaluation of Eight Clear Sky Models](../papers/atmosphere/2016-Qualitative%20and%20Quantitative%20Evaluation%20of%20Eight%20Clear%20Sky%20Models.pdf)** ([arXiv](https://arxiv.org/abs/1612.04336)) | Comparison framework for assumptions, dimensional consistency, performance, and numeric/visual validation rather than another production renderer. |
| B | 2022 | C. A. Gueymard, M. Kocifaj — **[Clear-Sky Spectral Radiance Modeling Under Variable Aerosol Conditions](https://doi.org/10.1016/j.rser.2022.112901)** *(public author PDF not found)* | Measured aerosol variability and spectral sky radiance can validate terrestrial presets and expose the limits of a single fixed aerosol profile. It informs parameters rather than the realtime integration architecture. |
| B | 2022 | P. Vevoda et al. — **[A Wide Spectral Range Sky Radiance Model](../papers/atmosphere/2022-Wide%20Spectral%20Range%20Sky%20Radiance%20Model.pdf)** ([DOI](https://doi.org/10.1111/cgf.14677)) | Extends fitted clear-sky radiance from ultraviolet through near infrared. It is relevant to spectral validation, solar energy, sensors, and future non-visible rendering, not the current three-channel realtime path. |
| B | 2024 | M. Kolarova, L. Lachiver, A. Wilkie — **[An Empirically Derived Adjustable Model for Particle Size Distributions in Advection Fog](../papers/atmosphere/2024-Adjustable%20Particle%20Size%20Distributions%20in%20Advection%20Fog.pdf)** ([DOI](https://doi.org/10.1111/cgf.15008)) | Fits measured droplet-size distributions with humidity, altitude, and visibility controls and supplies source material. This is a practical physical parameterization for a future local-weather volume, not a replacement for clear-air planetary scattering. |
| B | 2025 | P. Satilmis, T. Bashford-Rogers — **[A Multi-Timescale Image-Space Model for Dynamic Cloud Illumination](../papers/atmosphere/2025-Multi-Timescale%20Image%20Space%20Model%20for%20Dynamic%20Cloud%20Illumination.pdf)** ([DOI](https://doi.org/10.1016/j.cag.2024.104124)) | Predicts coherent whole-sky cloud illumination over several timescales. Its image-space learned model is useful when moving clouds affect terrain lighting, but does not provide volumetric cloud geometry or planetary transport. |
| Watch | 2026 | P. Satilmis et al. — **[Image Based Whole Sky Cloud Volume Generation](https://doi.org/10.1109/TVCG.2025.3641982)** *(public author PDF not found)* | The meaningful forward citation of the dynamic-cloud paper: it reconstructs whole-sky cloud volumes rather than illumination images alone. Track it for future volumetric weather, but it does not alter the current clear-air atmosphere. |
| B | 2025 | S. Schneegans et al. — **[Physically Based Real-Time Rendering of Eclipses](../papers/atmosphere/2025-Physically%20Based%20Real-Time%20Rendering%20of%20Eclipses.pdf)** ([DOI](https://doi.org/10.1111/cgf.70017)) | Extends planetary atmospheric transport to extended solar sources, atmospheric refraction, umbra, and penumbra. It matters once the world has moons or eclipses, but does not change the present single-sun surface renderer. |
| B | 2026 | I. J. Maquignaz — **[Full Dynamic Range Sky-Modelling for Image-Based Lighting](../papers/atmosphere/2026-Full%20Dynamic%20Range%20Sky%20Modelling%20for%20Image%20Based%20Lighting.pdf)** ([arXiv](https://arxiv.org/abs/2603.05758)) | Produces high-dynamic-range sky maps and associated scene illumination from user controls. It is a comparison for atmosphere-derived terrain lighting and artistic controls, but its learned sky imagery is not a ground-to-orbit transport solution. |
...
| B | 2023 | Y. Dobashi, N. Ishikawa, K. Iwasaki — **[Efficient Visualization of Light Pollution for the Night Sky](https://doi.org/10.1145/3618337)** *(public author PDF not found)* | Adds many ground-based emitters and efficient night-sky transport. This is useful for future cities and settlements, but unrelated to the current sunlight-shadow defect. |
...
### Volumetric shadow and ray-marching specialization

These works are narrower than the planetary atmosphere models above, but they
directly address the current mountain-shadow shafts, sampling discontinuities,
temporal stability, and future multi-view reuse.

| Priority | Year | Work | Project relevance |
|:---:|---:|---|---|
| A | 2014 | O. Klehm, H.-P. Seidel, E. Eisemann — **[Prefiltered Single Scattering](https://doi.org/10.1145/2556700.2556704)** *(public author PDF not found)* | Introduces the prefiltered single-scattering formulation: filter the shadowed medium contribution over image footprints rather than blur final radiance. This is the clearest architectural lead for antialiased, temporally stable shafts, but the unavailable full text prevents it from being the sole implementation authority. |
| A | 2014 | O. Klehm, H.-P. Seidel, E. Eisemann — **[Filter-Based Real-Time Single Scattering Using Rectified Shadow Maps](../papers/atmosphere/2014-Filter-Based%20Real-Time%20Single%20Scattering.pdf)** ([JCGT](https://jcgt.org/published/0003/03/02/)) | Extends epipolar rectification with separable filtering of the participating medium and shadow map. It is the strongest accessible reference for a prefiltered comparison path and directly targets undersampling around volumetric shadow boundaries. |
| A | 2014 | H. Dou et al. — **[Adaptive Depth Bias for Shadow Maps](../papers/atmosphere/2014-Adaptive%20Depth%20Bias%20for%20Shadow%20Maps.pdf)** ([DOI](https://doi.org/10.1145/2556700.2556706)) | Computes a tight per-fragment bias from the receiver and nearby depth bounds instead of relying on a global constant or slope-scaled bias. It is directly useful for reducing detached mountain shadows and self-shadow acne without erasing thin silhouettes. |
| A | 2015 | C. Peters, R. Klein — **[Moment Shadow Mapping](../papers/atmosphere/2015-Moment%20Shadow%20Mapping.pdf)** ([supplement](../papers/atmosphere/2015-Moment%20Shadow%20Mapping%20Supplementary%20Document.pdf), [DOI](https://doi.org/10.1145/2699276.2699277)) | Establishes the four-moment lower-bound reconstruction and its numerical safeguards. The supplement supplies the derivations and shader listings needed to implement and test a filterable-visibility comparison rather than infer it from later extensions. |
| A | 2016 | C. Peters et al. — **[Beyond Hard Shadows: Moment Shadow Maps for Single Scattering, Soft Shadows and Translucent Occluders](../papers/atmosphere/2016-Beyond%20Hard%20Shadows.pdf)** ([supplement](../papers/atmosphere/2016-Beyond%20Hard%20Shadows%20Supplementary%20Document.pdf), [DOI](https://doi.org/10.1145/2856400.2856402), [code](https://www.momentsingraphics.de/I3D2016.html)) | Uses moment shadow maps as a filterable visibility representation for single scattering. It is a strong experiment for smooth penumbrae and stable shaft filtering, provided light leaking and moment bias are measured against the current binary-depth oracle. |
| A | 2017 | C. Peters et al. — **[Improved Moment Shadow Maps for Translucent Occluders, Soft Shadows and Single Scattering](../papers/atmosphere/2017-Improved%20Moment%20Shadow%20Maps.pdf)** ([JCGT](https://jcgt.org/published/0006/01/03/)) | The self-contained journal extension improves numerical robustness and quantization and is the preferred implementation reference for any moment-based experiment. It does not remove the need to qualify leakage at thin mountain silhouettes. |
| A | 2017 | C. Peters — **[Non-Linearly Quantized Moment Shadow Maps](../papers/atmosphere/2017-Nonlinearly%20Quantized%20Moment%20Shadow%20Maps.pdf)** ([supplement](../papers/atmosphere/2017-Nonlinearly%20Quantized%20Moment%20Shadow%20Maps%20Supplementary%20Document.pdf), [DOI](https://doi.org/10.1145/3105762.3105775)) | Replaces linear moment storage with a nonlinear quantization that substantially reduces 64-bit rounding error and the bias needed to hide it. This is the practical compact-storage companion to the improved reconstruction, but leakage must still be measured explicitly. |
| B synthesis | 2018 | C. Peters — **[Moment-Based Methods for Real-Time Shadows and Fast Transient Imaging](../papers/atmosphere/2018-Moment%20Based%20Methods%20for%20Real%20Time%20Shadows%20and%20Fast%20Transient%20Imaging.pdf)** | This German-language dissertation summary unifies the moment-reconstruction theory behind the shadow papers and time-of-flight imaging. It is useful for understanding numerical assumptions and failure modes, while the original papers and supplements remain the implementation references. |
| A analysis | 2014 | L. Belcour, K. Bala, C. Soler — **[A Local Frequency Analysis of Light Scattering and Absorption](../papers/atmosphere/2014-Local%20Frequency%20Analysis%20of%20Light%20Scattering%20and%20Absorption.pdf)** ([DOI](https://doi.org/10.1145/2629490)) | Derives local bandwidth through scattering and absorption. Use it to choose filter footprints from transport bandwidth instead of applying one fixed screen-space blur to every depth and sun angle. |
| B comparison | 2011 | C. Wyman — **[Voxelized Shadow Volumes](../papers/atmosphere/2011-Voxelized%20Shadow%20Volumes.pdf)** ([DOI](https://doi.org/10.1145/2018323.2018329)) | Voxelizes shadow volumes so many rays can query volumetric visibility cheaply. It is a useful world-space, view-independent comparison, but memory and voxel resolution make it less attractive than the existing fitted-depth hierarchy for sparse planetary terrain. |
| B comparison | 2012 | J. Hanika et al. — **[Camera Space Volumetric Shadows](../papers/atmosphere/2012-Camera%20Space%20Volumetric%20Shadows.pdf)** ([DOI](https://doi.org/10.1145/2370919.2370921)) | Stores and filters volumetric occlusion in camera space for production rendering. Its reconstruction and filtering ideas are useful references, while camera dependence conflicts with the renderer's view-stable planet/sun-relative atmosphere caches. |
| B comparison | 2013 | C. Wyman, Z. Dai — **[Imperfect Voxelized Shadow Volumes](../papers/atmosphere/2013-Imperfect%20Voxelized%20Shadow%20Volumes.pdf)** ([DOI](https://doi.org/10.1145/2492045.2492050)) | Relaxes exact voxelized shadow volumes and filters the imperfect representation to gain speed. It offers a bounded-quality comparison for approximate world-space visibility, not the first production choice for crisp mountain silhouettes. |
| A | 2010 | T. Engelhardt, C. Dachsbacher — **[Epipolar Sampling for Shadows and Crepuscular Rays in Participating Media with Single Scattering](https://publikationen.bibliothek.kit.edu/1000026862)** ([DOI](https://doi.org/10.1145/1730804.1730823), *public local PDF not found*) | Establishes epipolar sampling for concentrating work along the image-space structure of light shafts. It is the conceptual baseline for the later rectification and hierarchy papers. |
| A | 2010 | I. Baran et al. — **[A Hierarchical Volumetric Shadow Algorithm for Single Scattering](../papers/atmosphere/2010-Hierarchical%20Volumetric%20Shadow%20Algorithm%20for%20Single%20Scattering.pdf)** ([DOI](https://doi.org/10.1145/1882262.1866200)) | Rectifies view and light rays into an epipolar coordinate system, then evaluates visibility with a partial-sum tree. The reported 17-120x CPU acceleration shows how much regularity the rectification exposes. |
| A | 2011 | J. Chen et al. — **[Real-Time Volumetric Shadows Using One-Dimensional Min-Max Mipmaps](../papers/atmosphere/2011-Real-Time%20Volumetric%20Shadows%20Using%20One-Dimensional%20Min-Max%20Mipmaps.pdf)** ([DOI](https://doi.org/10.1145/1944745.1944752)) | Treats each rectified shadow-map row as a one-dimensional height field and uses a min-max hierarchy to find lit intervals in parallel on the GPU. This is directly applicable to sharper, cheaper terrain-shadowed shafts. |
| A | 2013 | H.-Y. Lin et al. — **[Adaptive Sampling Approach for Volumetric Shadows in Dynamic Scenes](../papers/atmosphere/2013-Adaptive%20Sampling%20Approach%20for%20Volumetric%20Shadows%20in%20Dynamic%20Scenes.pdf)** ([DOI](https://doi.org/10.1049/iet-ipr.2013.0067)) | Uses shadow volumes and a stencil history to identify pixels invalidated by dynamic occluders and retain prior results elsewhere. Its discontinuity mask and selective temporal update are relevant to stable camera motion. |
| A | 2014 | A. Muñoz — **[Higher Order Ray Marching](../papers/atmosphere/2014-Higher%20Order%20Ray%20Marching.pdf)** ([DOI](https://doi.org/10.1111/cgf.12424)) | Compares numerical integration schemes for participating media. Higher order alone fails at visibility discontinuities; recursive adaptive quadrature is the useful lesson for resolving sharp mountain-shadow boundaries without uniformly increasing samples. |
| A | 2020 | T. Shihan et al. — **[Adaptive Volumetric Light and Atmospheric Scattering](../papers/atmosphere/2020-Adaptive%20Volumetric%20Light%20and%20Atmospheric%20Scattering.pdf)** ([DOI](https://doi.org/10.1371/journal.pone.0242265)) | A terrain-oriented flight-simulator pipeline combining epipolar sampling, visible-factor culling, near/far sampling levels, lookup tables, and lit-segment acceleration. It is the closest end-to-end comparison for our current scene. |
| A | 2021 | T. Xu, W. Zeng, E. Wu — **[Viewport-Resolution Independent Anti-Aliased Ray Marching on Interior Faces in Cube-Map Space](../papers/atmosphere/2021-Viewport-Resolution%20Independent%20Anti-Aliased%20Ray%20Marching.pdf)** ([DOI](https://doi.org/10.1145/3478512.3488598)) | Performs marching in bounded-resolution cube-map space and resamples into the viewport, decoupling integration cost and anti-aliasing quality from display resolution. It offers a concrete way to stabilize shafts without tying their sampling grid to camera pixels. |
| B | 2023 | T. Xu et al. — **[Efficient Binocular Rendering of Volumetric Density Fields with Coupled Adaptive Cube-Map Ray Marching for Virtual Reality](https://doi.org/10.1109/TVCG.2023.3322416)** (*public local PDF not found*) | Extends the cube-map approach with interleaved multi-resolution sampling, stereo reuse, temporal accumulation, and mixed mesh-volume rendering. It is useful for future multi-view or virtual-reality rendering rather than the first single-view atmosphere fix. |
| A experiment | 2025 | M. Jakab — **[Volumetric Radial Shadow Maps](../papers/atmosphere/2025-Volumetric%20Radial%20Shadow%20Maps.pdf)** ([paper source](https://cescg.org/wp-content/uploads/2025/04/Volumetric-Radial-Shadow-Maps-1.pdf)) | Run-length encodes shadow intervals along epipolar spokes so ray cost follows depth complexity rather than a fixed step count. It is promising for our shafts, but is preliminary, non-peer-reviewed work with segment-capacity and near-camera quality limits. |
| B future media | 2025 | R. Kern, F. Brüll, T. Grosch — **[Real-Time Importance Deep Shadow Maps with Hardware Ray Tracing](../papers/atmosphere/2025-Real-Time%20Importance%20Deep%20Shadow%20Maps%20with%20Hardware%20Ray%20Tracing.pdf)** ([DOI](https://doi.org/10.1111/cgf.70178), [code](https://github.com/TU-Clausthal-Rendering/ImportanceDeepShadowMaps)) | Allocates deep-shadow samples according to the current view and stores them in a ray-tracing acceleration structure, reporting up to 6.89x speedup over tracing every deep visibility ray. It is relevant when clouds or other semitransparent media arrive, but its view-dependent cache is not a replacement for the current opaque-terrain min/max hierarchy. |
| B | 2025 | P. Stadlbauer et al. — **[Adaptive Multi-View Radiance Caching for Heterogeneous Participating Media](https://doi.org/10.1111/cgf.70051)** ([TU Graz record](https://tugraz.elsevierpure.com/en/publications/adaptive-multiview-radiance-caching-for-heterogeneous-participati/), *repository PDF could not be imported automatically*) | Shares fog and cloud lighting between viewers through a layered world-space spherical-harmonic probe grid. This is relevant to a future multiplayer renderer and persistent camera-independent atmosphere work. |
| B oracle | 2025 | D. Bauer et al. — **[GSCache: Real-Time Radiance Caching for Volume Path Tracing Using 3D Gaussian Splatting](../papers/atmosphere/2025-GSCache%20Real-Time%20Radiance%20Caching%20for%20Volume%20Path%20Tracing.pdf)** ([arXiv](https://arxiv.org/abs/2507.19718)) | Stores path-space radiance in three-dimensional Gaussian splats to improve low-sample volume path tracing. It is chiefly a future stochastic-rendering and reference-image technique, not the deterministic clear-air runtime path. |
| B systems | 2025 | W. Tatzgern et al. — **[Sparse Cache Updates for Scalable Distributed Effect-Based Rendering](https://doi.org/10.1145/3757377.3763968)** (*public local PDF not found*) | Selectively propagates cache changes in a multi-viewer, multi-GPU renderer, reducing inter-GPU bandwidth by 6-65x. It is relevant to eventual shared-world/cloud rendering, not the immediate atmospheric shadow integrator. |

Google Scholar's Hillaire citation set contained 29 records on 2026-08-28.
The research rows added or linked above cover every distinct renderer extension
in that set that affects this project. The remaining records are
application citations, image dehazing, daylight measurement, infrared scene
simulation, theses that reproduce Hillaire, or unrelated uses; importing them
would not add another atmosphere algorithm. The publisher-only light-pollution
paper remains linked but is not represented by a placeholder PDF.

The next citation pass covered the shadow-aware, variable-aerosol, adjustable
fog, dynamic-cloud, Eradiate, and full-dynamic-range works. The shadow-aware,
Eradiate, and full-dynamic-range papers had no forward citations yet. The fog
paper's sole citation concerns optical communications. Twenty-three
variable-aerosol citations were almost entirely solar-energy measurement and
forecasting; a modular all-sky scientific model is worth watching but does not
offer a realtime graphics algorithm. Of the dynamic-cloud paper's three
records, *Image Based Whole Sky Cloud Volume Generation* is the only distinct
renderer extension and is recorded above. No additional clear-air planetary
transport method emerged from this generation.
...
- Canonical subdivision works: **173**.
- Tier totals: **A=108, B=58, C=7**.
- Canonical table rows: **167**, plus **6** detailed cross-disciplinary records carrying merged subdivision metadata.
- Duplicate canonical IDs: **none**.
- Duplicate canonical titles: **none**.
- Stable direct public PDFs recorded: **114/173**.
...
- Hierarchy sequence: **H01–H38 represented**.
