...
- Fact: A P6 packet owner declares whether its world tetrahedron is reflected; the topology certificate swaps its first two template corners and proves all interior faces have opposite winding and exactly two uses, while every one-use face lies on the root domain.
- Evidence: Uniform, root-boundary, one-level, and adjacent two-level BCC fronts passed with zero invalid boundaries or nonmanifold faces; the initial certificate failure exposed the previously undeclared reflected orientation.
...
### gpu-hierarchy-selector-threshold-band
- Updated: 2026-09-05
- Tags: gpu-lod, vulkan, oracle, floating-point
...
### metal-p5a-fixed-atmosphere-target
- Updated: 2026-09-03
- Tags: metal, atmosphere, metalfx, temporal, performance
